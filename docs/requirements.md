...
| FR-044 | Eigener Punktestand            | Als Mitglied möchte ich meinen Saison- und Karriere-Punktestand auf einen Blick sehen, damit ich meinen Beitrag einordnen kann.                                     | High     | Implemented |
...
| FR-177 | Leaderboard nach Wertdimension filtern | Als Mitglied möchte ich die Rangliste auch nach einer ganzen Wertdimension («Ehrenamt») eingrenzen und die Säulen unter diesen Namen gruppiert sehen, damit Rangliste und Wertdimensionen dasselbe Vokabular tragen. | Medium   | Implemented |
...
| FR-198 | Eingeplante Punkte sichtbar    | Als Mitglied möchte ich neben dem Geleisteten sehen, was ich bereits zugesagt habe und noch gutgeschrieben wird – angemeldete Schichten, übernommene Aufgaben, mein Amt –, damit ich vor Saisonende weiss, ob mein Beitrag steht (UC-042). | Medium   | Partial – gebaut und in einer zurückgerollten Transaktion gegen die laufende Datenbank geprüft (35/35); `0103` ist **nicht eingespielt**. |
...
| FR-079 | Push-Benachrichtigung     | Als Mitglied möchte ich Push-Benachrichtigungen auf iOS, Android und im Browser erhalten, damit ich Wichtiges rechtzeitig sehe.                        | High     | Partial – Browser, PWA und iOS stehen (UC-052); die **Android-App** bleibt offen, solange kein ntfy-Dienst betrieben wird. |
...
| FR-188 | Puls als eigenes Mailblatt | Als Mitglied möchte ich den Vereins-Puls per E-Mail mit allen drei Abschnitten erhalten, damit ich ihn lesen kann, ohne die App zu öffnen (UC-050). | High | Implemented |
| FR-189 | Vorschau des Pulses | Als Vorstand möchte ich vor der Freigabe sehen, wie der Puls in der App und im Postfach ankommt, damit ich nichts versende, das ich nicht gesehen habe (UC-050). | High | Implemented |
| FR-190 | Beiträge im Puls | Als Mitglied möchte ich im Puls auch die vereinsweiten Beiträge aus App und Website der letzten zwei Wochen finden, damit die Wochenübersicht vollständig ist (UC-050). | Medium | Implemented |
| FR-191 | Grussformel am Amt | Als Vorstand möchte ich einen Grusstext an ein Vorstandsamt hängen, damit der Puls mit Name und Amt grüsst und die Unterschrift bei der Amtsübergabe mitwechselt (UC-050). | Medium | Implemented |
| FR-192 | Porträt zur Grussformel | Als Mitglied möchte ich zum Gruss ein Gesicht sehen, damit der Puls von Menschen kommt und nicht von einem System (UC-050). | Low | Implemented |
...
| ID     | Titel                       | User Story                                                                                                                                                     | Priority | Status |
| ------ | --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------ |
| FR-116 | Rechnungsmodul aktivieren   | Als Kassier:in möchte ich den Rechnungsdienst aus den Vereinseinstellungen aktivieren, damit wir Rechnungen stellen können, ohne die App zu wechseln.            | Medium   | Implemented |
| FR-117 | Mitglieder-Sync ans Billing | Als System möchte ich Änderungen an Mitgliederstammdaten an den Rechnungsdienst übermitteln, damit dessen Debitoren-Spiegel aktuell bleibt. **Seit dem Entscheid vom 14.09.2026 (UC-046) läuft der Dienst in nexus; der Ausgangskorb bleibt für einen künftigen externen Dienst bestehen.** | Low   | Partial |
...
| FR-119 | Punkte bei pünktlicher Zahlung | Als System möchte ich bei einer fristgerecht bezahlten Rechnung Punkte der Säule 6 buchen und sie der Wertdimension «Finanzen» zurechnen, damit die Achse eine Quelle hat statt dauerhaft «nicht erhoben» zu zeigen. | Medium   | Implemented |
| FR-169 | Gläubigerangaben des Vereins | Als Kassier:in möchte ich QR-IBAN, Name und Adresse unseres Vereins hinterlegen, damit jede Rechnung einen gültigen Einzahlungsschein trägt (UC-046).            | Medium   | Implemented |
| FR-170 | Abrechnungsperiode anlegen  | Als Kassier:in möchte ich eine Abrechnungsperiode mit Zweck und Fälligkeit anlegen, damit ich Beiträge gebündelt stellen kann (UC-046).                          | Medium   | Implemented |
| FR-171 | Beitrag und Zuschläge pflegen | Als Kassier:in möchte ich Beiträge je Team und wiederkehrende Zuschläge und Abzüge pflegen, damit die Positionen einer Rechnung nicht von Hand entstehen (UC-046). | Medium   | Implemented |
| FR-172 | Rechnungsentwürfe erzeugen  | Als Kassier:in möchte ich für die gewählten Mitglieder Entwürfe mit Positionen und Betrag erzeugen, damit ich sie vor dem Versand prüfen kann (UC-046).           | Medium   | Implemented |
| FR-173 | QR-Rechnung als PDF         | Als System möchte ich je Rechnung ein PDF mit Schweizer QR-Einzahlungsschein und geprüfter Zahlungsreferenz erzeugen, damit sie bei jeder Schweizer Bank zahlbar ist (UC-046). | High     | Implemented |
| FR-174 | Rechnung versenden          | Als Kassier:in möchte ich geprüfte Entwürfe versenden, damit jedes Mitglied sein PDF per E-Mail erhält und die Rechnung in der App sieht (UC-046).                | High     | Implemented |
| FR-175 | Zahlungseingänge abgleichen | Als Kassier:in möchte ich die Zahlungsdatei unserer Bank hochladen, damit bezahlte Rechnungen über ihre Zahlungsreferenz von selbst als bezahlt gelten.           | High     | Implemented |
| FR-176 | Zahlungserinnerung senden   | Als Kassier:in möchte ich an eine offene Rechnung erinnern, damit ich nicht einzeln nachfassen muss (UC-046 A8).                                                  | Medium   | Implemented |
...
| FR-199 | Push-Versand                | Als Mitglied möchte ich, dass eine Meldung mich auch dann erreicht, wenn die App zu ist – über den Browser, die PWA oder die iOS-App und ohne Google-Dienst (UC-052). | High     | Partial – gebaut und gegen erfundene Gegenstellen geprüft; **noch kein Push hat ein echtes Gerät erreicht**: Migration, Deploy, Secrets und Gerätetest stehen aus. |
| FR-200 | Profil einrichten (Assistent) | Als Mitglied möchte ich nach dem Beitritt einmal durch Bild, Name, Erreichbarkeit und Meldungen geführt werden, damit mein Profil nicht die halbe E-Mail-Adresse bleibt (UC-053). | Medium   | Partial – gebaut und gegen die laufende Datenbank geprüft; `0105` ist **nicht eingespielt**. |
...
| NFR-040 | Anmeldemail übersteht den eigenen Versand | Der Hook, der Anmeldemails im Vereins-Look verschickt, antwortet innerhalb des Fünf-Sekunden-Budgets von Supabase Auth und schickt die Mail danach weiter (`EdgeRuntime.waitUntil`); Sprache, Verein und Logo fallen einzeln auf einen Standard zurück, ein gescheiterter Versand steht im Log der Function, und der Hook lässt sich in `config.toml` abschalten, worauf GoTrue wieder selbst verschickt. | Reliability     | High     | Implemented   |
---
...
| C-003 | Keine Google-Abhängigkeit      | Firebase, FCM, Google Maps, Google ML Kit und Google-Login sind ausgeschlossen; auch «Sign in with Apple» entfällt dadurch. Die Schriften kommen aus dem Bündel, nicht von Googles Schriftserver.                              | Technical   | High     | Implemented |
| C-004 | Push-Kanäle                    | Push läuft über ntfy/UnifiedPush (Android), APNs (iOS) und Web Push VAPID (PWA), mit der In-App-Inbox als vollständigem Fallback.                        | Technical   | High     | Partial – APNs und Web Push VAPID sind gebaut und geprüft (UC-052); ntfy fehlt, weil kein Dienst betrieben wird. |
