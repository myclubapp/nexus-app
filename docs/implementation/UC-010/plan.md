...
| 2026-09-12 | **FR-156, Schritt 6:** Nach der Zusage bietet das Detail den Termin dem Kalender des Geräts an, und wer zugesagt hat, findet die Zeile «In den Kalender eintragen» unter «Mein Status». `src/lib/calendar.ts` baut den Eintrag (Titel, Ort, Fenster in Unix-ms, Notizen aus Terminart und Warum; ohne Ende eine Stunde, BR-187) und wählt den Weg: nativ `createEventWithPrompt` von `@ebarooni/capacitor-calendar` (Systemblatt, keine Berechtigung), im Browser ICS-Download mit UTC-Zeiten. Info.plist trägt die drei `NSCalendars…`-Zweckangaben, weil das Plugin EventKit verlinkt. Tests: `calendar.test.ts` (11), zwei in `EventDetailModal.test.tsx`. Offen: Plattform-Parität auf dem Gerät (Testplan TC-010). |
| 2026-09-13 | **BR-196:** Bei einem Termin mit Schichten gibt es keine Zusage zum Anlass – «Mein Status», Wischen, die drei Listen und «Erinnern» entfallen; die Agenda-Zeile zeigt den Haken, wenn man eine Schicht hält, sonst einen Platzhalter. Auslöser: ein Helfereinsatz mit 7/7 besetzten Schichten liess sich weiter «zusagen». Regel in `canRespond()`/`canRemind()`, Details im Nachtrag zu UC-012. |
